...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'WakaTime');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'summaries',
    description: 'Fetch WakaTime daily coding summaries for a user over a date range (start/end required, YYYY-MM-DD); returns per-day breakdowns by project, language, editor, OS, and category in seconds.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'stats',
    description: 'Fetch aggregated WakaTime coding stats for a user over a range ("last_7_days", "last_30_days", "last_6_months", "last_year", "all_time"); returns totals and rankings by language, project, editor, and OS.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'durations',
    description: 'Fetch WakaTime coding durations (start/end timestamps with project and file) for a single day (date required, YYYY-MM-DD); optionally slice_by entity/language/branch for granular session data.',
    inputSchema: {
      type: 'object',
  ...
  { name: 'leaders', description: 'Fetch WakaTime\'s public coding leaderboard; optionally filter by language or country_code and paginate; returns ranked users with display names and weekly coding totals.', inputSchema: { type: 'object', properties: { language: { type: 'string' }, country_code: { type: 'string' }, page: { type: 'number' } } } },
  {
    name: 'commits',
    description: 'List the commits WakaTime has recorded for one project belonging to a WakaTime user, given user_id (or "current") and the project name, with optional author, branch, and page. Returns each commit hash, message, author, branch, timestamps, and the coding time WakaTime attributes to it. Answers what has been committed to a project tracked by WakaTime and how much editor time went into it.',
    inputSchema: {
      type: 'object',
  ...
  { name: 'editors', description: 'Fetch WakaTime\'s public editor usage statistics showing relative popularity of code editors and IDEs across all WakaTime users.', inputSchema: { type: 'object', properties: {} } },
  { name: 'meta', description: 'Fetch WakaTime API metadata including current server time and available timezones; useful for validating timezone strings before passing them to summaries or durations.', inputSchema: { type: 'object', properties: {} } },
];

async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const apiKey = (args._apiKey as string | undefined)?.trim();
  if (!apiKey) throw new Error('WakaTime requires an API key: pass your key as the _apiKey argument (free at https://wakatime.com/api-key).');
  ...
  const get = async (path: string, params?: Record<string, unknown>) => {
    ...
    const res = await pwFetch(`${BASE}${path}${[...p].length ? `?${p}` : ''}`, {
    ...
    if (!res.ok) throw await httpError(res, 'WakaTime');
    return res.json();
  };
